<template>
  ...
  <Dialog 
    :header="`編輯「${perkName}」`" 
    v-model:visible="refIsEdit" :breakpoints="{'960px': '75vw', '640px': '90vw'}" 
    :style="{width: '50vw'}" :model="true"
  >
    <div class="flex align-items-center">
      <h3>Perk Name：</h3>
...
const refIsEdit = ref(props.isEdit);
const refIsDisplay = ref(props.isDisplay);

onUpdated(()=>{
  if(refIsEdit.value){
    perkName.value = props.perkData.name;
    perkID.value = props.perkData.id;
    if(!defaulted.value){
      state.newPerkName = props.perkData.name;
      state.newPerkInfor = props.perkData.illustrate;
      state.newPerkUseful = props.perkData.usefulness;
...
const complete = (perk, list) => {
  emits("complete", props.perkIndex);
